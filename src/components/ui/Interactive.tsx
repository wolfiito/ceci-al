...
interface InteractiveProps extends HTMLMotionProps<"div"> {
  children: ReactNode;
  className?: string;
  isCard?: boolean; // Si es true, el efecto es más sutil (para tarjetas grandes)
}
...
export default function Interactive({ 
  ...
  // Configuración para Botones (Más agresiva)
  const buttonVariants = {
    hover: { scale: 1.05, filter: "brightness(1.05)" },
    tap: { scale: 0.95, filter: "brightness(0.95)" }
  };

  // Configuración para Tarjetas (Más sutil para no marear)
  const cardVariants = {
